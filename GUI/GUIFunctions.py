...
def popupMessage(question):

    # Destroy popups.
    def leaveMini(answer):
        popup.destroy()
        return answer
    ...
    label = ttk.Label(popup, text=question, font=normalFont)
    label.pack()
    response = ttk.Entry(popup)
    response.pack()
    button1 = ttk.Button(popup, text="Okay", command=lambda: leaveMini(response.get()))
    button1.pack()
    popup.mainloop()
