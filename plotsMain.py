...
def main(parameters):
    ...
    '''FILTERING'''
    filterbankband = [[12],[30]]
    # fs =250
    eegTemp = eeg_testing  # nu afmetingen eeg: shape eeg (24, 5*120)
    eeg = eeg_testing
    eeg = np.zeros((eeg.shape[0], len(PARAMETERS["filterBankband"][0]), eeg.shape[1], eeg.shape[2]), dtype=np.float32)
    for band in range(len(PARAMETERS["filterBankband"][0])):
        lower, upper = scipy.signal.iirfilter(8, np.array(
            [2 * PARAMETERS["filterBankband"][0, band] / fs, 2 * PARAMETERS["filterBankband"][1, band] / fs]))
        eeg[:, band, :, :] = np.transpose(scipy.signal.filtfilt(lower, upper, np.transpose(eegTemp), axis=0))
    ...
    for i in range(np.shape(eeg_testing)[0]):
        eegfeatminute = eeg_testing[i]
        for j in range(int(60/decisionWindow)):
            eegfeat = eegfeatminute[:,:,j*decisionWindow*fs:(j+1)*decisionWindow*fs]
            # print(np.shape(eegfeat))
